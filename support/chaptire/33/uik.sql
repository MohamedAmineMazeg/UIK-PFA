...
--
-- Hôte : 127.0.0.1
-- Généré le : mer. 12 jan. 2022 à 01:43
...
(1, 3, 'Math1', 'Math1', NULL, 1, NULL, NULL, NULL),
(2, 3, 'Math2', 'Math2', NULL, 1, NULL, NULL, NULL);
...
INSERT INTO `cour` (`id`, `libelle`, `description`, `id_semester`, `id_annee_scolaire`, `id_enseignant`, `duree`, `id_section`, `credits`, `dt_cour`, `id_groupe`) VALUES
(3, 'Algébre 1', '', 1, 1, 1, '45', 3, 4, '2022-01-10', 2),
(4, 'Analyse 1', 'Analyse 1', 1, 1, 1, '45', 3, 4, '2022-01-09', 1),
(5, 'Algébre 2', 'Algébre 2', 2, 1, 1, '45', 4, 4, '2022-01-12', 2),
(6, 'Analyse 2', 'Analyse 2', 2, 1, 1, '45', 4, 4, '2022-01-13', 2),
(14, 'Java J2EE', '', 1, 1, 1, '24', NULL, 6, '2022-01-11', 2);
...
INSERT INTO `cour_salle` (`id`, `id_cour`, `id_salle`, `dt_cour`, `id_groupe`) VALUES
(1, 3, 1, '2022-01-10', 1),
(2, 4, 2, '2022-01-10', 1),
(3, 3, 1, '2022-01-16', 1),
(4, 4, 3, '2022-01-17', 1),
...
CREATE TABLE `enseignant` (
  ...
  `dt_diplome` varchar(100) DEFAULT NULL,
...
INSERT INTO `enseignant` (`id`, `diplome`, `dt_diplome`, `experience`, `specialite`, `id_users`, `universite`) VALUES
(1, 'Diplome', '2012-12-12', 10, 'Spécialite', 5, 'PhD graduate of Oregon Staate University'),
(2, 'Diplome', '2012-12-12', 10, 'Spécialite', 6, 'PhD graduate of Oregon Staate University'),
(3, 'Diplome', '2012-12-12', 10, 'Spécialite', 7, 'PhD graduate of Oregon Staate University'),
(4, 'Diplome', '2012-12-12', 10, 'Spécialite', 8, 'PhD graduate of Oregon Staate University');
...
INSERT INTO `etudiant` (`id`, `id_user`) VALUES
(1, 4);
...
(1, 11, 'Prep-Int-G01', 1, 1),
(2, 11, 'Prep-Int-G02', 1, 1),
(3, 13, 'Ing-01-G01', 1, 2),
(6, 13, 'Ing-01-G02', 1, 2),
(7, 13, 'Ing-02-G01', 1, 1),
(8, 13, 'Ing-02-G02', 1, 2),
(9, 13, 'Ing-03-G01', 1, 3),
(10, 13, 'Ing-03-G02', 1, 3);
...
INSERT INTO `groupe_etudiant` (`id`, `id_etudiant`, `id_groupe`) VALUES
(1, 1, 1);
...
INSERT INTO `rendu` (`id`, `libelle`, `dt_depot`, `dt_dealine`, `support`, `support_name`, `id_chaptir`) VALUES
(2, 'Math 1', '2022-01-11', '2022-01-20', '/UIK/rendu/2/Math1.txt', 'Math1.txt', 1),
(3, 'Math 1', '2022-01-15', '2022-01-26', '/UIK/rendu/3/Math2.txt', 'Math2.txt', 2);
...
INSERT INTO `section` (`id`, `libelle`, `id_barnche`) VALUES
(3, 'Mathématique 1', 11),
(4, 'Mathématique 2', 11),
(5, 'Informatique 1', 11),
(6, 'Informatique 2', 11),
(7, 'Sciences ingénieur 1', 11),
(8, 'Sciences ingénieur 2', 11),
(9, 'Langue et soft skills', 11);
...
(5, 'Ben Ali', 'Mahmoud', 'mahmoud.uik@uik.com', 'prof1', 'aa', 3, '/UIK/photo/prof/60b781fa41975.jpg', '55632411', 'Tunis', '1950-08-08', 1, 1),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_cour_salle`  AS SELECT `cs`.`id` AS `id`, `cs`.`dt_cour` AS `dt_cour`, `cs`.`id_cour` AS `id_cour`, `c`.`libelle` AS `cour_libelle`, `c`.`id_enseignant` AS `id_enseignant`, `e`.`id_users` AS `id_users`, `cs`.`id_groupe` AS `id_groupe`, `g`.`libelle` AS `groupe_libelle` FROM (((((`cour_salle` `cs` join `salle` `s` on(`s`.`id` = `cs`.`id_salle`)) join `cour` `c` on(`c`.`id` = `cs`.`id_cour`)) join `enseignant` `e` on(`e`.`id` = `c`.`id_enseignant`)) join `groupe` `g` on(`g`.`id` = `cs`.`id_groupe`)) join `annee_scolaire` `as2` on(`as2`.`id` = `c`.`id_annee_scolaire` and `as2`.`actif` = '1')) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_examen`  AS SELECT `e`.`id` AS `id`, `e`.`libelle` AS `examen_libelle`, `e`.`coefficient` AS `coefficient`, `e`.`duree` AS `duree`, `e`.`pourcentage` AS `pourcentage`, `e`.`dt_debut` AS `dt_debut`, `e`.`dt_fin` AS `dt_fin`, `e`.`dt_examen` AS `dt_examen`, `e`.`id_type_examen` AS `id_type_examen`, `te`.`libelle` AS `type_examen_libelle`, `e`.`id_cour` AS `id_cour`, `c`.`libelle` AS `cour_libelle`, `c`.`id_semester` AS `id_semester`, `s`.`libelle` AS `semester_libelle`, `c`.`id_annee_scolaire` AS `id_annee_scolaire`, `ge`.`id_groupe` AS `id_groupe`, `g`.`libelle` AS `groupe_libelle`, `c`.`id_enseignant` AS `id_enseignant`, `e2`.`id_users` AS `id_users`, `e`.`id_salle` AS `id_salle`, `sa`.`libelle` AS `salle_libelle` FROM ((((((((((`examen` `e` join `type_examen` `te` on(`te`.`id` = `e`.`id_type_examen`)) join `cour` `c` on(`c`.`id` = `e`.`id_cour`)) join `section` `s2` on(`s2`.`id` = `c`.`id_section`)) join `branche` `b` on(`b`.`id` = `s2`.`id_barnche`)) join `semestre` `s` on(`s`.`id` = `c`.`id_semester`)) join `annee_scolaire` `as2` on(`as2`.`id` = `c`.`id_annee_scolaire` and `as2`.`actif` = '1')) join `enseignant` `e2` on(`e2`.`id` = `c`.`id_enseignant`)) join `groupe_enseignant` `ge` on(`ge`.`id_enseignant` = `e2`.`id`)) join `groupe` `g` on(`g`.`id` = `ge`.`id_groupe` and `g`.`id_branche` = `b`.`id`)) join `salle` `sa` on(`sa`.`id` = `e`.`id_salle`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_rendu_prof`  AS SELECT `r`.`id` AS `id`, `r`.`libelle` AS `rendu_libelle`, `r`.`dt_depot` AS `dt_depot`, `r`.`support` AS `support`, `r`.`support_name` AS `support_name`, `r`.`id_chaptir` AS `id_chaptir`, `c`.`libelle` AS `chaptir_libelle`, `c`.`couverture` AS `couverture`, `c`.`id_cour` AS `id_cour`, `c2`.`libelle` AS `cour_libelle`, `c2`.`id_semester` AS `id_semester`, `s`.`libelle` AS `semester_libelle`, `c2`.`id_enseignant` AS `id_enseignant`, `e`.`id_users` AS `id_users` FROM (((((`rendu` `r` join `chapitre` `c` on(`c`.`id` = `r`.`id_chaptir`)) join `cour` `c2` on(`c2`.`id` = `c`.`id_cour`)) join `annee_scolaire` `as2` on(`as2`.`id` = `c2`.`id_annee_scolaire` and `as2`.`actif` = '1')) join `semestre` `s` on(`s`.`id` = `c2`.`id_semester`)) join `enseignant` `e` on(`e`.`id` = `c2`.`id_enseignant`)) ;
...
ALTER TABLE `cour_salle`
  ...
  ADD KEY `cour_salle_FK_2` (`id_groupe`);
...
ALTER TABLE `historique_etudiant`
  ADD PRIMARY KEY (`id`),
  ADD KEY `historique_etudiant_FK` (`id_groupe_etud`),
  ADD KEY `historique_etudiant_FK_1` (`id_situation_etudiant`);
...
ALTER TABLE `commentaire`
  ...
  ADD CONSTRAINT `commentaire_FK_1` FOREIGN KEY (`id_groupe_etud`) REFERENCES `historique_etudiant` (`id`);
...
ALTER TABLE `cour`
  ...
  ADD CONSTRAINT `cour_FK_4` FOREIGN KEY (`id_section`) REFERENCES `section` (`id`);
...
ALTER TABLE `cour_salle`
  ...
  ADD CONSTRAINT `cour_salle_FK_2` FOREIGN KEY (`id_groupe`) REFERENCES `groupe` (`id`);
...
--
ALTER TABLE `historique_etudiant`
  ADD CONSTRAINT `historique_etudiant_FK` FOREIGN KEY (`id_groupe_etud`) REFERENCES `groupe_etudiant` (`id`),
  ADD CONSTRAINT `historique_etudiant_FK_1` FOREIGN KEY (`id_situation_etudiant`) REFERENCES `situation_etudiant` (`id`);
...
--
ALTER TABLE `section`
  ADD CONSTRAINT `section_FK` FOREIGN KEY (`id_barnche`) REFERENCES `branche` (`id`);
